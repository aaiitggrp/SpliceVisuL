...
data_dir= '../../data/version_%s_26_intronic_type2' % version
save_dir = './save_lstm_base_v%s/' % version
file_path = '../../checkpoints/v%s/lstm_base' % version
log_path = os.path.join(file_path, 'logs')
print(up_down_stream)
file_train = 'train_v%s_%s.h5' % (version, up_down_stream)
file_val = 'val_v%s_%s.h5' % (version, up_down_stream)
...
#######################################################################################################################################
model = Sequential()
model.add(Embedding(max_features, 100, input_shape=[max_features]))
...
model.save('lstm_base_%s_%s_%d_final.h5' % (month, day, up_down_stream))
os.chdir(work_dir)
